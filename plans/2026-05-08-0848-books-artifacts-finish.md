...
- `runtime/books/<topic>/<book-slug>/` — локальный рабочий комплект с оригиналом и копией toolkit, повторяющий тематическую иерархию tracked `books/`;
...
- Как пользователь Books, я вижу оригинал в `runtime/books/<topic>/<book-slug>/` в main-worktree и могу использовать его для уточнения концептов.
