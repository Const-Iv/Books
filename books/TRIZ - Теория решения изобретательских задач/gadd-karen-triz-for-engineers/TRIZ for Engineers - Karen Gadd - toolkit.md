...
Источник:
- Structured Markdown source copy для уточнения концептов: [runtime/books/TRIZ - Теория решения изобретательских задач/gadd-karen-triz-for-engineers/Karen Gadd - TRIZ for Engineers.md](<../../../runtime/books/TRIZ - Теория решения изобретательских задач/gadd-karen-triz-for-engineers/Karen Gadd - TRIZ for Engineers.md>)
...
6. Если нужно уточнить контекст по книге, открой [structured Markdown source copy](<../../../runtime/books/TRIZ - Теория решения изобретательских задач/gadd-karen-triz-for-engineers/Karen Gadd - TRIZ for Engineers.md>); `Topic index` подскажет, где искать.
...
Пример из книги:
- Gadd показывает это на инженерных сессиях: пока команда говорит про drain, material или component, она уже выбрала решение. Ideal Outcome возвращает вопрос к benefits: что должно стать правдой для системы без прежних затрат и вреда. Только после этого можно искать functions и resources.
...
Пример из книги:
- В книге Bad Solution Park нужен, чтобы не убивать ранние идеи и одновременно не позволять им управлять сессией. Например, идеи вроде новых drains или тяжелых доработок сначала паркуются. Затем из них извлекают needs, harms и contradictions.
...
Пример из книги:
- 9-Boxes раскрывается через примеры вроде red kindergarten table и инженерных case studies: проблема видна иначе в прошлом, будущем, subsystem и super-system. Локальный дефект может иметь причину до события и решение вне самого объекта. Поэтому карта времени и масштаба расширяет поле решения.
...
Пример из книги:
- Gadd предлагает брать bad solution и спрашивать: что она улучшает и что ухудшает. Так бытовые и инженерные примеры вроде hot/cold cup или large/small bag превращаются в contradiction. После этого включаются matrix или separation, а не спор о вкусе решения.
...
Пример из книги:
- В книге physical contradiction хорошо виден на простых примерах: cup должен быть hot около напитка и not hot для руки; umbrella должна быть large in use and small in bag. Это учит искать разделение по месту, времени или условию. Компромисс становится не первым, а последним вариантом.
...
Пример из книги:
- В примере с проблемой puddles на производстве команда сначала думала о новых drains. WHY поднял обсуждение к нужному benefit, а HOW спустил его к functions and resources. Так проблема перестала быть закупкой и стала поиском способа выполнять clean process без прежнего вреда.
...
Пример из книги:
- Gadd связывает X-Factor с Ideal Outcome: сначала неизвестный исполнитель описывается функцией, а потом ищется среди ресурсов. В примере с bagless vacuum прорыв приходит через уже существующий принцип разделения потока, а не через добавление мешка. Смысл в поиске доступного носителя функции.
...
Пример из книги:
- Trimming в книге используется как low-cost route: убрать компонент или process step, но сохранить output. В пищевом примере с burnt edges команда не обязана начинать с дорогой trimming machine; сначала проверяется, кто еще может устранить harm или выполнить нужную функцию. Это и есть ruthless pass.
...
Пример из книги:
- Function Analysis у Gadd переводит систему в Subject-Action-Object. Пример toothbrush that damages gums показывает, что один и тот же объект может выполнять полезную и вредную функции. Карта нужна, чтобы лечить конкретное действие, а не весь продукт сразу.
...
Пример из книги:
- Standard Solutions у Gadd выбираются после problem type. Если toothbrush damages gums, это harm; если полезное действие слабое, это insufficiency; если проблема в контроле, это measurement. Разные классы ведут к разным conceptual routes.
...
Пример из книги:
- 8 Trends в книге используются для next-generation thinking: bagless vacuum, mobile phones, aircraft examples и другие системы рассматриваются как движение к большей ideality, dynamism или segmentation. Trend scan помогает не только чинить дефект, но и увидеть следующий продуктовый ход.
...
Пример из книги:
- Smart Little People и Size-Time-Cost у Gadd нужны против psychological inertia. Когда обычная схема не двигается, problem zone превращается в поведение малых участников, а масштаб времени, размера и цены намеренно искажается. Это дает новые варианты без прежней формы.
...
Пример из книги:
- BAE Systems case study показывает, зачем нужен audit trail: команда проходит от problem context к Ideal Outcome, contradictions, selected tools and concepts. Когда решение потом выглядит очевидным, путь все равно остается видимым. Это помогает защищать инженерный выбор.
...
Пример из книги:
- Chapter 13 собирает problem-solving maps: не каждый кейс требует всех инструментов. В BAE-style workflow сначала определяется тип проблемы, затем выбирается tactic. Map защищает от попытки применить весь TRIZ сразу.
...
Пример из книги:
- ARIZ в книге вынесен как heavy route для non-trivial tasks. Если быстрые tools не дали сильного решения, алгоритм возвращает команду к problem definition, contradictions, mini-problem, resources and conceptual solutions. Это резерв дисциплины, а не ежедневная форма.
...
Пример из книги:
- В инженерных примерах Gadd решение оценивается не по количеству новых функций, а по балансу benefits, costs and harms. Если новая деталь дает benefit, но резко увеличивает complexity или risk, ideality может не вырасти. Поэтому формула заставляет считать вреды вместе с выгодами.
...
Пример из книги:
- В drainage/puddles example команда сначала видит решение как новые drains. Через Ideal Outcome она формулирует нужный benefit, затем функции clean process и ресурсы, которые уже есть в системе. Так route идет сверху вниз: от результата к исполнителю.
...
Пример из книги:
- Red kindergarten table показывает, что одна и та же проблема имеет прошлые причины, текущий контакт и будущие последствия. 9-Boxes заставляет искать решения до, во время и после события, а также на уровне subsystem, system и super-system. Поэтому карта расширяет набор вариантов.
...
Пример из книги:
- Bad Solution Park сохраняет ранние идеи как сырье. В книге они не выбрасываются: из них извлекают good aspects, bad aspects and contradictions. Это снижает защиту авторов идеи и повышает качество problem model.
...
Пример из книги:
- Coffee cup, plastic bag and umbrella examples показывают два уровня contradiction. Technical contradiction говорит, что одно улучшается и другое ухудшается; physical contradiction говорит, что одному объекту нужны противоположные свойства. Это учит выбирать matrix или separation по типу конфликта.
...
Пример из книги:
- Cup hot/cold and umbrella large/small examples показывают разделение. Свойство может быть нужно не всегда, не везде или не на одном уровне системы. Поэтому вопрос о времени, месте, условии и уровне часто сильнее среднего компромисса.
...
Пример из книги:
- Toothbrush example показывает Function Analysis как карту действий: brush removes plaque, but can damage gums. Важна не вещь toothbrush сама по себе, а конкретные useful and harmful functions. Тогда решение ищется для действия.
...
Пример из книги:
- Oxford Standard Solutions в книге начинаются с классификации: harm, insufficiency or measurement. Для harm доступны блокирование, устранение, превращение в пользу или correction afterwards. Для weak function route другой, поэтому problem type критичен.
...
Пример из книги:
- Trends у Gadd помогают читать развитие систем: bagless vacuum moves toward higher ideality, mobile devices show size/function tension, aircraft examples show dynamics and fields. Это framework для next version, а не только для текущего дефекта.
...
Пример из книги:
- ARIZ example layer показывает, что строгий алгоритм нужен, когда задача не раскладывается быстрыми tools. Он удерживает contradiction, mini-problem and resources, пока не появится conceptual solution. Поэтому ARIZ работает как disciplined escalation.
