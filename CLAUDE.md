...
- Каждый новый Books toolkit должен содержать в главном файле после `Быстрая карта` раздел `Лайфхаки, приемы и инструменты к внедрению`, извлечённый из всей книги. Пункты оформляются как карточки `Что внедрить`, `Контекст`, `Когда применять`, `Первый шаг`, `Источник / где искать в книге`; `Контекст` кратко передает конкретную source-backed сцену, кейс, мысленный эксперимент или причинную демонстрацию из книги (ситуация и участники, действие/решение, эффект), не повторяет идею и не выдумывает отсутствующие детали. Пользовательский раздел не называется `Белки`.
...
- Task-level решения формулировать через charter, цель, контекст, одну owner-language Job Story, входные данные, ожидаемый результат, критерии приемки и проверку; отдельные task-level JTBD/User Story не добавлять.
