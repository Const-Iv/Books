...
Каждый новый Books toolkit должен содержать в главном файле после `Быстрая карта` раздел `Лайфхаки, приемы и инструменты к внедрению`, извлечённый из всей книги. Пункты раздела оформляются как карточки `Что внедрить`, `Контекст`, `Когда применять`, `Первый шаг`, `Источник / где искать в книге`; `Контекст` кратко передает конкретную source-backed сцену, кейс, мысленный эксперимент или причинную демонстрацию (ситуация и участники, действие/решение, эффект), не повторяет идею и не выдумывает отсутствующие детали. Раздел не называется `Белки`, потому что это только исходная метафора из методики Маргулана.

Каждый toolkit также проходит micro-practice coverage: named concepts, авторские метафоры, чек-листы, ритуалы, инструменты, practical imperative phrases and nested actionable subheadings из source получают статус `card | folded_into | excluded_with_reason`. Конкретная практика не должна молча исчезать внутри более крупного framework.
...
- Adaptive Subagent Workflow описан в `.memory-bank/subagent-workflow.md`: семиусловный fail-closed Triviality Gate выбирает `single | sequential | parallel/hybrid`; overlapping writers и параллельные live actions запрещены, а bounded subagent возвращает `findings`, `evidence`, `risks`, `unresolved`, `recommended next step`.
