---
name: gh-fix-ci
description: Inspect failing GitHub PR checks with the GitHub CLI, fetch GitHub Actions logs, summarize actionable failure context, and prepare a local fix plan before code changes. Use when a user asks Codex to debug or fix failing GitHub CI checks for the current branch or a specific pull request.
metadata:
...
- PR number or URL, optional. If omitted, resolve the PR for the current branch.
...
3. Summarize only actionable facts:
   - Failing check name.
...
4. Propose the smallest fix path:
