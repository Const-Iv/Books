...
- Каждый новый Books toolkit должен содержать в главном файле после `Быстрая карта` раздел `Лайфхаки, приемы и инструменты к внедрению`, извлечённый из всей книги. Пункты раздела оформляются как карточки `Что внедрить`, `Контекст`, `Когда применять`, `Первый шаг`, `Источник / где искать в книге`; `Контекст` кратко передает concrete source-backed ситуацию, действие/решение и эффект, не повторяет идею и не выдумывает отсутствующие детали. Пользовательское название `Белки` не используется.
- Micro-practice coverage обязателен: named concepts, авторские метафоры, чек-листы, ритуалы, инструменты, practical imperative phrases and nested actionable subheadings из source получают статус `card | folded_into | excluded_with_reason`, чтобы конкретные практики не исчезали внутри крупных framework'ов.
