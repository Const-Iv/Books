...
- Approved Books charter center: книга превращается в применимый русскоязычный toolkit, а не в обычное summary. Books-specific изменения должны усиливать `extract structure, not summaries`.
- Books toolkit output должен строиться вокруг reusable элементов: модели, принципы, техники, лайфхаки / приемы / инструменты к внедрению, anti-patterns, сценарии применения, шпаргалки, glossary, patterns / techniques and topic index.
- Каждый новый Books toolkit должен содержать в главном файле после `Быстрая карта` раздел `Лайфхаки, приемы и инструменты к внедрению`, извлечённый из всей книги. Каждый пункт раздела оформлять как прикладную карточку `Что внедрить`, `Когда применять`, `Первый шаг`, `Источник / где искать в книге`; не называть пользовательский раздел `Белки`.
- Нельзя принимать изменение, которое превращает Books в простой пересказ, создаёт дословное воспроизведение книги как product output, смешивает идеи книги с профессиональной консультацией или ослабляет переносимость baseline, deterministic QA, safe task flow and source-of-truth governance.
- Books v1 runtime утверждён 2026-05-07 как local CLI contour on Node/npm orchestration with optional Python extraction adapter. Product source должен жить под `src/books/`; generated local artifacts — под ignored `runtime/books/<topic>/<book-slug>/`.
...
- Входные книги могут быть на разных языках, но output первой версии Books всегда русский.
...
- Для старта нового проекта после owner-authored миссии ассистент должен предлагать следующий intake-пункт сам и просить owner'а подтвердить или поправить формулировку, а не задавать каждый следующий пункт как пустой вопрос.
