...
**Service layout:** применимо и утверждено для локального прототипа. Future product source: `src/books/`; shareable toolkit artifacts: tracked `books/<topic>/<book-slug>/`; локальные originals / generated artifacts: ignored `runtime/books/`; tests: `tests/unit` / `tests/integration`; process scripts stay in `scripts/`.
