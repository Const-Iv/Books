...
- сохранять локальный комплект с оригиналом и копией toolkit в ignored `runtime/books/<topic>/<book-slug>/`, чтобы локальные originals повторяли тематическую иерархию shareable toolkit'ов и по исходному тексту можно было быстро уточнять концепты;
