...
- Books artifacts rule: tracked `books/<topic>/<book-slug>/` хранит shareable toolkit и manifest без полного оригинала; ignored `runtime/books/<topic>/<book-slug>/` хранит same-basename structured Markdown source copy и рабочие artifacts с такой же тематической иерархией, а `task:finish:core` переносит весь `runtime/books` tree в main перед cleanup. Имена исходника и `.md` copy совпадают по basename `<Автор> - <Название>`; toolkit source links ведут на локальный `.md` path plus heading/page/spine marker. Original retention: сохранять original рядом с `.md` для `pdf`, `epub`, `fb2` и audio; для остальных форматов после verified `.md` оригинал/extracted `.txt` в Books runtime не нужен, если owner отдельно не попросил сохранить такой оригинал.
- Ideal Books toolkit rule: каждый single-book и multi-book toolkit должен идти по master-format из `src/books/toolkit/toolkit-contract.mjs` и образца `books/TRIZ - Теория решения изобретательских задач/triz-unified-practical-toolkit/Единый практический toolkit TRIZ - по нескольким книгам.md`: usage layer, `Battle route`, `Training route`, `Быстрая карта`, `Tool selector`, action cards, deep reference body, coverage/source notes, limitations, anti-patterns, scenarios, cheatsheet, glossary and topic index.
- Books micro-practice rule: каждый source-derived named concept, авторская метафора, чек-лист, ритуал, инструмент, practical imperative phrase или nested actionable subheading должен получить статус `card | folded_into | excluded_with_reason`; silent loss внутри крупного framework считается QA failure.
- Multi-book Books rule: если toolkit делается из нескольких книг, сначала подробно сделать и сохранить standalone toolkit по каждой книге, затем собирать combined toolkit под owner-selected идеей или темой. Combined depth берётся напрямую из local structured Markdown source copies/originals; standalone toolkit'ы используются как coverage-control artifacts, not depth ceiling. Combined должен покрывать все достойные внимания идеи, не урезать справочную глубину, убирать повторы, сохранять coverage/source traceability и идти в master-практической последовательности. Для Books качество разбора важнее скорости, времени и экономии токенов.
- `task:start` и shared `$worktree-create` не должны считать запуск `codex app <worktreePath>` доказательством открытого Codex-чата. `openedChat=true` допустим только после read-back локального Codex thread state с exact `cwd` нового worktree; при отсутствии matching thread нужно сохранять и показывать `openAttempted`, `openStatus=unverified|failed|skipped`, `openDiagnostics`, `openCommand` и не выдавать успешный статус открытия.
- Books action-card context rule: каждая новая карточка раздела внедрения содержит `Контекст` — сжатый source-backed summary конкретной ситуации/участников, действия или решения и эффекта из книги. Контекст не повторяет `Что внедрить`, не заменяется generic advice, не выдумывает отсутствующие детали и не воспроизводит большой фрагмент source.
