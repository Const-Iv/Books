...
- `skills/starter-project-bootstrap/` — основной project-local skill для conversational bootstrap нового downstream-проекта после копирования или подключения starter baseline. Интенты `стартуем новый проект`, `запусти новый проект`, `проведи bootstrap нового проекта`, `я скопировал starter в новый репозиторий` должны вызывать этот skill: на clean `main` ассистент автоматически создаёт managed bootstrap worktree, затем устанавливает зависимости при необходимости, выполняет `npm run skills:link`, определяет bootstrap state, проводит Project Intake Gate, переносит approved ответы в canonical sources и запускает baseline QA. Если `skills:link` требует `--adopt` из-за конфликтующих локальных skill targets, ассистент должен показать конкретные target paths и запросить separate explicit owner approval; нельзя объединять worktree approval и `--adopt` approval в один вопрос. Feature/refactor/behavior-change work запрещён до approved intake.
