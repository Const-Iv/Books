...
- Каждый новый Books toolkit должен содержать в главном файле после `Быстрая карта` раздел `Лайфхаки, приемы и инструменты к внедрению`, извлечённый из всей книги. Каждый пункт пишется как прикладная карточка `Что внедрить`, `Контекст`, `Когда применять`, `Первый шаг`, `Источник / где искать в книге`; `Контекст` кратко передает конкретную source-backed сцену, кейс, мысленный эксперимент или причинную демонстрацию из книги (ситуация и участники, действие/решение, эффект), не повторяет идею и не выдумывает отсутствующие детали. Пользовательский раздел не называется `Белки`, потому что это только исходная метафора из методики Маргулана.
...
- `starter.agent.adaptive-subagent-workflow`: Каждый нетривиальный plan/spec проходит семиусловный fail-closed Triviality Gate по `.memory-bank/subagent-workflow.md` и выбирает `single | sequential | parallel/hybrid`; overlapping writers и параллельные live actions запрещены.
...
- `starter.conveyor.goal-seed-handoff`: Goal Seed является стандартным self-contained prompt для task conveyor: цель, исходные project sources, `Definition of Done`, зона влияния, safety boundaries, проверки, UI oracle, governance/eval и stop conditions. `task:start` по умолчанию формирует и сохраняет effective Goal Seed; deep-link `prompt=<effectiveSeed>` означает только заполненный composer draft. Отправленный turn подтверждается persisted exact `cwd + first_user_message` read-back; raw seed допустим только через `--no-goal-seed`.
- `starter.conveyor.worktree-create-open-only`: `$worktree-create` по умолчанию создаёт managed worktree и открывает exact workspace в Codex без `prompt=` и без `codex_app.create_thread`; владелец сам пишет следующую задачу в открытой папке. Только явный owner request стартовать отдельную задачу разрешает `--native-handoff` и app-native local thread в уже созданном managed worktree.
...
## Shared Starter Baseline Rules — synced 2026-08-10

- `starter.conveyor.successor-lineage-cleanup`: Переписанный после публикации task result удаляется только по отдельно одобренному ignored exact-SHA manifest. Version 1 сохраняет ordered finished/published successor task/commit/path records; version=2 добавляет typed `managed_task` / `approved_direct_main`, где direct commit обязан быть single-parent на first-parent canonical `main`, не иметь task state и объявлять exact rewritten `paths` плюс full `changedPaths` внутри process-only profile. Complete accounting всех rewritten paths/commits, transport-only merges, immutable manifest/proof/history, original/current-main QA и обычные dependency/local-only/profile/pre/post runtime/read-back gates обязательны; PASS — `superseded_verified`, а не equivalence.
- `starter.conveyor.legacy-state-reconciliation`: Legacy task-state перед cleanup восстанавливается только по отдельно одобренному ignored exact-SHA manifest. Missing `baseSha` восстанавливается только как sole parent exact task tip; existing recorded `baseSha` manifest повторяет без замены, а Git доказывает непустую strict linear single-parent chain до task tip. Exact task/commit/sealed-main, clean worktree, exact QA/preview и ordered MERGE/PUSH/MAIN_VERIFY tuple обязательны. Retry сверяет exact manifest path/hash, SHA-addressed immutable proof, recovered state и reconciliation history, затем воспроизводит proof против sealed main, который обязан оставаться ancestor current main; status-флаг не является proof. Ручная правка state запрещена; `merged -> finished` фиксирует cleanup как `failed`, а обычные successor/dependency/local-only/profile/post-cleanup gates сохраняются.
- `starter.conveyor.successor-lineage-refresh`: Sealed successor-lineage state и связанное proof evidence нельзя менять, обнулять или заменять вручную. Если canonical `main` продвинулся до cleanup, refresh допускается только отдельным explicit cleanup-only flow с новым ignored manifest: прежние state, manifest, proof и history проверяются как immutable; новый `main` должен быть descendant ранее подтверждённого `main`; порядок и непрерывность прежних successor records сохраняются; original/current-main QA и все обычные dependency, local-only, profile и runtime/read-back gates повторяются. Старый proof сохраняется неизменным, новый результат добавляется append-only; любой gap блокирует cleanup.
